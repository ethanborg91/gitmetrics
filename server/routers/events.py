...
SCHEMA = json.loads((pathlib.Path(__file__).parent.parent / "schemas" / "event-v1.json").read_text())  # Path adjusted for modularity

@router.post("/")
async def ingest(event: dict, current_user: User = Depends(get_current_user)):
    try:
